<?php
    ...
    $page = str_replace("compiledForm.html","#start".$ID,$page);
    $dbAccess = new DBAccess();
    $connection = $dbAccess->openDBConnection();
    if($connection) {    
        ...
        if(move_uploaded_file($_FILES['Immagine']['tmp_name'],$ImmagineUpload)) {
            ...
            if($insertion) {
                ...
                $page = str_replace('valueHashtag',$Hashtag,$page);
                $page = str_replace('valueAction','insert.php',$page);
                $page = str_replace('<input type="file" id="Immagine" name="Immagine" accept="image/*" title="Immagine o foto del risultato finale della ricetta" aria-label="Foto della ricetta"/>', '<input type="file" id="Immagine" name="Immagine" accept="image/*" required="required" title="Immagine o foto del risultato finale della ricetta" aria-label="Foto della ricetta"/>',$page);
            } 
        }
        else {
            $page = str_replace("Errore","Errore nel caricamento",$page);
